...
"""
This is the logging facility of the openvre-agent-api. It is meant to provide
a unified way for Agents to log information that needs to be read by the
...
DEBUG:   Detailed information, typically of interest only when
         diagnosing problems.
INFO:    Confirmation that Agent execution is working as expected.
WARNING: An indication that something unexpected happened, but that the
         Agent can continue working successfully.
ERROR:   A more serious problem has occurred, and the Agent will not be
         able to perform some function.
FATAL:   A serious error, indicating that the Agent may be unable to
         continue running.

As well as the following non-standard levels:

PROGRESS: Provide the VRE with information about Agent execution progress.
...
def progress(message, *args, **kwargs):
    """
    Provides information about Agent progress.

    Logs a message containing information about Agent progress, with level
    ``PROGRESS``.
    ...
    .. code-block:: python
       ...
       class TestAgent(Agent):
           def run(self, input_files, input_metadata, output_files):
               logger.progress("TestAgent starting", status="RUNNING")
               total_tasks = 3

               self.task1()
               logger.progress("TestAgent", task_id=1, total=total_tasks)

               self.task2()
               logger.progress("TestAgent", task_id=2, total=total_tasks)

               self.task3()
               logger.progress("TestAgent", task_id=3, total=total_tasks)

               logger.progress("TestAgent", status="DONE")
               return True
